...
import type { Asset, AssetKind, Clip, ProjectSnapshot, TextLayer } from "./types";
import {
  ...
  return new Promise((resolve, reject) => {
    ...
    el.addEventListener(
      ...
      () => {
        ...
        window.setTimeout(() => done(Number.isFinite(el.duration) ? el.duration : 0), 2500);
      },
      { once: true },
    );
    el.src = url;
...
async function makeThumbnail(url: string, kind: AssetKind): Promise<string | undefined> {
...
export function VideoEditor() {
  ...
  const [lastExport, setLastExport] = useState<{ url: string; name: string; size: number } | null>(
    null,
  );
  const [settings, setSettings] = useState({ format: "mp4" as "mp4" | "webm", fps: 30, bitrate: 12 });
  ...
  const addClip = useCallback((asset: Asset, track: number) => {
    setClips((prev) => {
      const endOfTrack = prev
        ...
        .reduce((max, clip) => Math.max(max, clip.start + clipDuration(clip)), 0);
      const sourceDuration = asset.duration > 0 ? asset.duration : DEFAULT_IMAGE_DURATION;
      const clip: Clip = {
        id: uid("clip"),
        assetId: asset.id,
        track,
  ...
  const importFiles = useCallback(
    async (files: File[]) => {
      ...
      for (const file of usable) {
        ...
        try {
          const meta =
            kind === "image" ? await readImageMeta(url) : await readMediaMeta(url, kind);
          ...
          const asset: Asset = {
            ...
            kind,
            url,
            duration: kind === "image" ? DEFAULT_IMAGE_DURATION : meta.duration || 1,
            width: meta.width,
            height: meta.height,
            size: file.size,
            thumbnail,
  ...
  const patchClip = useCallback(
    (patch: Partial<Clip>) => {
      ...
      setClips((prev) =>
        prev.map((clip) => (clip.id === selectedId ? { ...clip, ...patch } : clip)),
      );
    },
    [selectedId],
  );
  ...
  const patchText = useCallback(
    (patch: Partial<TextLayer>) => {
      if (!selectedId) return;
      setTexts((prev) =>
        prev.map((layer) => (layer.id === selectedId ? { ...layer, ...patch } : layer)),
      );
    },
    [selectedId],
  );
  ...
  const splitAtPlayhead = useCallback(() => {
    ...
    const right: Clip = { ...clip, id: uid("clip"), start: time, inPoint: sourceSplit };
    setClips((prev) => prev.flatMap((item) => (item.id === clip.id ? [left, right] : [item])));
  }, [pushHistory, selectedId, time]);

  const addTextLayer = useCallback(() => {
    pushHistory();
    const start = duration > 0 ? Math.min(time, Math.max(0, duration - 0.2)) : 0;
    const end = start + (duration > 0 ? Math.min(4, Math.max(0.5, duration - start)) : 4);
  ...
  const runExport = async () => {
    ...
    if (typeof MediaRecorder === "undefined") {
      setError("This browser has no MediaRecorder support — try Chrome or Edge.");
      return;
    }
    ...
    >
      <header className="flex h-12 shrink-0 items-center gap-3 border-b border-border px-3">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <Button size="sm" variant="outline" onClick={undo} disabled={historyDepth === 0}>
            Undo
          </Button>
          <Button size="sm" onClick={() => setExportOpen(true)} disabled={duration <= 0 || exporting}>
            Export
          </Button>
        </div>
      </header>
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="w-60 shrink-0 overflow-y-auto border-r border-border bg-background/60 p-2">
          <div className="mb-2 flex items-center justify-between px-1">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
              Media
            </p>
            <Button size="icon-xs" variant="ghost" onClick={() => fileInputRef.current?.click()}>
              +
            </Button>
          </div>
          {assets.length === 0 ? (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex gap-1">
                        <Button
                          size="xs"
                          variant="secondary"
                          onClick={() => addClip(asset, asset.kind === "audio" ? 1 : 0)}
                        >
                          Add
                        </Button>
                        <Button
        ...
        <main className="flex min-h-0 flex-1 flex-col">
          <div className="relative flex min-h-0 flex-1 items-center justify-center bg-black/85 p-4">
            ...
            {clips.length === 0 && texts.length === 0 ? (
              <div className="absolute max-w-sm rounded-lg border border-dashed border-border bg-background/80 p-8 text-center">
                <p className="text-sm font-medium">Kuch import karke editing shuru karo</p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Video, audio ya image drop karo — sab browser me hi rehta hai, koi upload nahi.
                </p>
          ...
          <div className="flex h-14 shrink-0 items-center gap-2 border-t border-border px-3">
            <Button size="icon" variant="ghost" onClick={() => seek(0)} title="Start">
              ⏮
            </Button>
            <Button size="icon" variant="ghost" onClick={() => seek(time - 1)} title="-1s">
              ◀◀
            </Button>
            <Button size="icon" variant="default" onClick={togglePlay} title="Play / pause (space)">
              {playing ? "❚❚" : "▶"}
            </Button>
            <Button size="icon" variant="ghost" onClick={() => seek(time + 1)} title="+1s">
              ▶▶
            </Button>
            <Button size="icon" variant="ghost" onClick={() => seek(duration)} title="End">
              ⏭
            </Button>
            ...
            <div className="ml-auto flex items-center gap-2">
              <div className="flex items-center gap-1">
                <Button
                  size="xs"
                  variant="outline"
                  onClick={() => setPxPerSec((value) => clamp(value / 1.4, 4, 400))}
                >
                  −
                </Button>
        ...
        <aside className="w-72 shrink-0 overflow-y-auto border-l border-border bg-background/60">
          <Inspector
            clip={selectedClip}
            text={selectedText}
            asset={selectedClip ? assetsById.get(selectedClip.assetId) ?? null : null}
            duration={duration}
        ...
        <div className="absolute inset-0 z-50 grid place-items-center bg-black/70 backdrop-blur-sm">
          <div className="w-80 space-y-3 rounded-lg border border-border bg-card p-5">
            ...
            <p className="text-[11px] text-muted-foreground">
              {formatTimecode(time)} / {formatTimecode(duration)} · real time, is tab ko visible
              rakho.
            </p>
            <Button
              size="sm"
        ...
        <div className="absolute inset-0 z-50 grid place-items-center bg-black/70 backdrop-blur-sm">
          <div className="w-96 space-y-4 rounded-lg border border-border bg-card p-5">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                {formatTimecode(duration)} · {compositionSize.width}×{compositionSize.height}
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-1.5">
                {BITRATES.map((bitrate) => (
                  <Button
                    key={bitrate.value}
                    size="sm"
                    variant={settings.bitrate === bitrate.value ? "default" : "outline"}
                    onClick={() => setSettings((prev) => ({ ...prev, bitrate: bitrate.value }))}
                  >
                    {bitrate.label}
                  </Button>
                ))}
            ...
            {lastExport ? (
              ...
              >
                Previous export download karo ({(lastExport.size / 1e6).toFixed(1)} MB)
              </a>
            ) : null}
