  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-[11px] text-muted-foreground">
        <span>{label}</span>
        {value ? <span className="tabular-nums text-foreground/80">{value}</span> : null}
      </div>
      {children}
    </div>
  );
...
const COLOR_SWATCHES = ["#ffffff", "#000000", "#ffd400", "#ff4d4d", "#31d158", "#4da3ff"];
  ...
  if (!clip && !text) {
    return (
      <div className="space-y-2 p-3 text-xs text-muted-foreground">
        ...
        <p>Koi clip ya text select karo — trimming, speed, volume, filters yahin milenge.</p>
        <ul className="list-disc space-y-1 pl-4">
          <li>Timeline par clip khinch kar move karo</li>
          <li>Clip ke kinaare (handles) se trim karo</li>
          <li>
            <kbd className="rounded bg-muted px-1">S</kbd> se playhead par split,{" "}
            <kbd className="rounded bg-muted px-1">Del</kbd> se delete
          </li>
        </ul>
      </div>
    );
      ...
      <div className="space-y-4 p-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Field label="Start (s)">
            <Input
              ...
              onChange={(event) =>
                onPatchText({
                  start: clamp(Number.parseFloat(event.target.value) || 0, 0, text.end - 0.1),
                })
              }
            />
          </Field>
          <Field label="End (s)">
            <Input
              ...
              onChange={(event) =>
                onPatchText({
                  end: clamp(Number.parseFloat(event.target.value) || 0.1, text.start + 0.1, Math.max(duration, text.start + 0.2)),
                })
              }
            />
          </Field>
        ...
        <Field label="Font size" value={`${Math.round(text.size)}px`}>
          <Slider
            ...
            max={220}
            step={1}
            onValueChange={(value) => onPatchText({ size: Array.isArray(value) ? value[0] : value })}
          />
        </Field>
        ...
        <div className="grid grid-cols-2 gap-2">
          <Field label="Position X" value={text.x.toFixed(2)}>
            <Slider
              ...
              max={1}
              step={0.01}
              onValueChange={(value) => onPatchText({ x: Array.isArray(value) ? value[0] : value })}
            />
          </Field>
          <Field label="Position Y" value={text.y.toFixed(2)}>
            <Slider
              ...
              max={1}
              step={0.01}
              onValueChange={(value) => onPatchText({ y: Array.isArray(value) ? value[0] : value })}
            />
          </Field>
        </div>
    ...
    <div className="space-y-4 p-3">
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-xs font-semibold">{asset?.name ?? "Clip"}</p>
          <p className="text-[11px] text-muted-foreground">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Field label="In (source s)">
          <Input
            ...
            onChange={(event) => setNumber("inPoint", event.target.value, 0, clip!.outPoint - 0.1)}
          />
        </Field>
        <Field label="Out (source s)">
          <Input
            ...
            onChange={(event) =>
              setNumber("outPoint", event.target.value, clip!.inPoint + 0.1, sourceDuration || 1e6)
            }
          />
        </Field>
      ...
      <Field label="Speed" value={`${clip!.speed.toFixed(2)}x`}>
        <Slider
          ...
          max={3}
          step={0.05}
          onValueChange={(value) => onPatchClip({ speed: Array.isArray(value) ? value[0] : value })}
        />
      </Field>
      ...
      <Field label="Volume" value={`${Math.round(clip!.volume * 100)}%`}>
        <Slider
          ...
          max={2}
          step={0.05}
          onValueChange={(value) => onPatchClip({ volume: Array.isArray(value) ? value[0] : value })}
        />
      </Field>

      <div className="space-y-3 rounded-md border border-border/70 p-2">
        <p className="text-[11px] font-medium text-muted-foreground">Colour adjust</p>
        <Field label="Brightness" value={clip!.brightness.toFixed(2)}>
          <Slider
            ...
            max={2}
            step={0.02}
            onValueChange={(value) => onPatchClip({ brightness: Array.isArray(value) ? value[0] : value })}
          />
        </Field>
        <Field label="Contrast" value={clip!.contrast.toFixed(2)}>
          <Slider
            ...
            max={2}
            step={0.02}
            onValueChange={(value) => onPatchClip({ contrast: Array.isArray(value) ? value[0] : value })}
          />
        </Field>
        <Field label="Saturation" value={clip!.saturate.toFixed(2)}>
          <Slider
            ...
            max={3}
            step={0.02}
            onValueChange={(value) => onPatchClip({ saturate: Array.isArray(value) ? value[0] : value })}
          />
        </Field>
        <Field label="Blur" value={`${clip!.blur.toFixed(1)}px`}>
          <Slider
            ...
            max={20}
            step={0.5}
            onValueChange={(value) => onPatchClip({ blur: Array.isArray(value) ? value[0] : value })}
          />
        </Field>
        <Button
          size="xs"
